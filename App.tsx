...
import { Book, Chapter, VoiceOption, ProcessingStatus, SyncState, SyncProvider } from '@/types';
import { URDU_VOICES } from '@/constants';
...
import { BookOpen, PlusCircle, Library as LibraryIcon, ShieldCheck, Loader2, Settings2, Cloud, Play, Pause, Sparkles } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const refreshLibrary = useCallback(async (silent = false) => {
    ...
    try {
      ...
      if (err.message === "UNAUTHORIZED") {
        alert("GitHub session expired or token revoked. Please reconnect.");
  ...
  const handleChapterPlay = async (chapter: Chapter) => {
    ...
    try {
      ...
      const chapterIdx = updatedBook.chapters.findIndex(c => c.id === chapter.id);
      updatedBook.chapters[chapterIdx] = { ...chapter, audioUrl };
      
      setSelectedBook(updatedBook);
      setActiveChapter(updatedBook.chapters[chapterIdx]);
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-xs font-bold uppercase tracking-[0.2em] text-[#d4af37]">Your Collection</h3>
              </div>
              <button 
            ...
            <div className="pt-8 border-t border-slate-900">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${syncState.provider !== 'none' ? 'bg-green-500/10 text-green-500' : 'bg-slate-900 text-slate-600'}`}>
                    <Cloud size={16} />
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Library Sync</p>
                    <p className="text-[9px] text-slate-600 mt-0.5">
                      {syncState.provider === 'github' ? 'Connected to GitHub Gist' : 
                       syncState.provider === 'public' ? `Connected to Room ${syncState.roomId}` : 
                       'Local Storage Only'}
                    </p>
                  </div>
                </div>
              ...
              <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="space-y-6">
                  {selectedBook.chapters.map((chapter) => (
                    <div 
                      key={chapter.id}
                      className="group p-6 md:p-8 bg-slate-950/50 border border-slate-900 rounded-2xl hover:border-slate-800 transition-all"
                    >
